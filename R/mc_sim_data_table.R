...
mcmc_input_files<-function(location,
  ...
  for (i in 1:length(species_list)){
    size<-rtnorm(n = 1000,
           mean = w_fish[c(w_fish$loc == location &
                             w_fish$Seasonality == season &
                             w_fish$SppAlias == species_list[i]), "mean_mass"],
           # sd = 0.1,
           lower = as.numeric(w_fish[c(w_fish$loc == location &
                              w_fish$Seasonality == season &
                              w_fish$SppAlias == species_list[i]), "min_mass"]),
           upper = as.numeric(w_fish[c(w_fish$loc == location &
                              w_fish$Seasonality == season &
                              w_fish$SppAlias == species_list[i]), "max_mass"]),
           sd = c(as.numeric(w_fish[c(w_fish$loc == location &
                             w_fish$Seasonality == season &
                              w_fish$SppAlias == species_list[i]), "min_mass"])+
             as.numeric(w_fish[c(w_fish$loc == location &
                              w_fish$Seasonality == season &
                              w_fish$SppAlias == species_list[i]), "max_mass"])) /2)
...
Bas = c(0.1,0.1,0.7,0.1,0)
  ...
  if(!is.null(settings_modelDiet)){
      ...
      AllData.0<-food_web_bioaccumulation(
                    settings = settings_obsDiet,
