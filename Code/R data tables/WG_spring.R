...
library(readxl)
library(here)
here::i_am(path = "./Code/R data tables/WG_spring.R")
...
# WG/NRP   as.Date("2020-11-10") (fall/winter)
            # as.Date("2021-04-28") (spring) # <<< this file, median values
...
d.fish<-d.fish %>%
  ...
  mutate(Seasonality = if_else(SampleDate > as.Date("2021-06-01"), "summer", 
                              if_else(SampleDate < as.Date("2020-11-20"), "fall/winter",
                                      "spring"))) %>% 
  filter(c(Tissue == "Whole Body" | Tissue == "Muscle") & Seasonality == "spring") %>% 
...
d.water<-d.water %>%
  ...
  summarize(PFHxS.m = median(PFHxS), PFHxS.min = min(PFHxS), PFHxS.max = max(PFHxS),
            ...
            PFDA.m = median(PFDA), PFDA.min = min(PFDA), PFDA.max = max(PFDA),
            PFUA.m = median(PFUA), PFUA.min = min(PFUA), PFUA.max = max(PFUA),
            temp.m = median(WaterTemp, na.rm = TRUE),
            temp.min = min(WaterTemp, na.rm = TRUE),
            temp.max = max(WaterTemp, na.rm = TRUE), 
            n = n())
...
Bas = c(0.1,0.7,0.1,0)
  ...
  C_OX = 8, # assumed
  T = 10, # unlist(c(d.water[, "temp.m"])), # no water temp assuming 10 C 
