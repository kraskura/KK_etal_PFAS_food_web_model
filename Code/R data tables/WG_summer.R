...
library(readxl)
library(here)
here::i_am(path = "./Code/R data tables/WG_summer.R")
...
d.fish<-d.fish %>%
  ...
  mutate(Seasonality = if_else(SampleDate > as.Date("2021-06-01"), "summer", 
                              if_else(SampleDate < as.Date("2020-11-20"), "fall/winter",
                                      "spring"))) %>% 
  filter(c(Tissue == "Whole Body" | Tissue == "Muscle") & Seasonality == "summer") %>% 
...
Bas = c(0.1,0.1,0.7,0.1,0)
  ...
  C_OX = 8, # assumed
  T = 23, # unlist(c(d.water[, "temp.m"])), no temp assuming 20 C 
