---
title: "Willow Grove and JBA new data"
...
```{r all datasets}
# add seasons 
# JBA Spring sampling (water data): March 30  - April 28 2021: 
# JBA Summer sampling (water data): June 14 - July 16 2021
# JBA Fall/Winter sampling (water data): November 19 2020 - January 14 2021

# WG Fall sampling (water data): October 9 - November 10 2020; second to last day 2020-11-05
# WG Summer sampling (water data): June 15 - August 19 (some sparse sampling) 2021
# WG Spring sampling (water data): February 9 - March 23 2021
...
ggsave(plot = fig2_jba_noPFOS, filename = "./Figures/SERDP/Fig2_jba_noPFOS.png", height = 3.5, width = 5,units = "in")
...
Willow Grove Water and Sediment
...
Willow Grove Water PFOS, PFHxS, PFOA
...
wg_sp1<-wg_all %>% 
  ...
  filter(Analyte %in% top12PFASwg[1:6]) %>%
  filter(!Analyte == "PFOS") %>% 
...
wg_sp2<-wg_all %>% 
  ...
  mutate(SppName = if_else(Spp_Loc == "bluegill", "BGL", 
                    if_else(Spp_Loc == "largemouth bass", "BAS", "small BGL"))) %>% 
  filter(Analyte %in% top12PFASwg[7:12]) %>%
  filter(!Analyte == "PFOS") %>% 
  dplyr::group_by(SppName, Analyte, Media, Seasonality) %>% 
  dplyr::summarise(meanVal = mean(value, na.rm = TRUE), 
                   sdVal = sd(value, na.rm = TRUE),
...
jba_sp1<-jba_all %>% 
  ...
  filter(Analyte %in% top12PFASjba[c(2:4, 6:8)]) %>%
  filter(!Analyte == "PFOS") %>% 
  dplyr::group_by(SppName, Analyte, Media, Seasonality) %>% 
  dplyr::summarise(meanVal = mean(value, na.rm = TRUE), 
                   sdVal = sd(value, na.rm = TRUE),
                       ...
                       if_else(Spp_Loc == "creek chubsucker", "CHU",
                        if_else(Spp_Loc == "eastern mudminnow", "MIN",
                         if_else(Spp_Loc == "fallfish", "FAL",
                          if_else(Spp_Loc == "swallowtail shiner", "SWA",
                           if_else(Spp_Loc == "margined madtom", "MAD", "PUM")))))))))) %>% 
  filter(Analyte %in% top12PFASjba[c(5, 9:12)]) %>%
  filter(!Analyte == "PFOS") %>% 
  dplyr::group_by(SppName, Analyte, Media, Seasonality) %>% 
  dplyr::summarise(meanVal = mean(value, na.rm = TRUE), 
                   sdVal = sd(value, na.rm = TRUE),
...
d_jba_wat_baf <- d_jba_wat %>%
  dplyr::select(SampleID, SampleDate, all_of(top12PFASjba)) %>% 
  pivot_longer(cols = all_of(top12PFASjba),
         names_to = "Analyte") %>%
  mutate(Media = "water",
         SampleDate = strptime(SampleDate, format = "%m/%d/%Y %H:%M")) %>% 
  mutate(Seasonality = if_else(SampleDate > as.Date("2021-06-10"), "summer", 
...
for(i in 1:length(jbaPFAS)){
  ...
  mod<-PFAS_lmers(data = d_jba_biota, system = "JBA", Analyte = jbaPFAS[i])[[2]]
  chi2<-as.data.frame(mod[1])
  df<-as.data.frame(mod[2])
  p<-as.data.frame(mod[3])
  
  if(i == 1){
    d_new<-cbind(chi2, df, p, rep(x = "JBA", 4), Analyte = rep(jbaPFAS[i], 4))
    d_new$Predictor<-rownames(d_new)
  }else{
    d_new0<-cbind(chi2, df, p, rep(x = "JBA", 4), Analyte = rep(jbaPFAS[i], 4))
    d_new0$Predictor<-rownames(d_new0)
    d_new<-rbind(d_new, d_new0)
  }
