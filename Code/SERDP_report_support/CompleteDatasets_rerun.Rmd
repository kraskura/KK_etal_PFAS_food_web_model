...
d_analytes<-read.csv(here("./Data/Analyte_chain_len.csv"))

# import data last checked: Jan 2025 -----
d_jba_biota<-read_xlsx(here("./Data/FINAL/JBA_Biota_Data.xlsx"), sheet = "Data")
d_jba_wat<-read_xlsx(here("./Data/FINAL/JBA_Water_Data.xlsx"), sheet = "Data")
d_jba_sed<-read_xlsx(here("./Data/FINAL/JBA_Sediment_Data.xlsx"), sheet = "Data")

d_wg_biota<-read_xlsx(here("./Data/FINAL/WG_Biota_Data.xlsx"), sheet = "Data")
d_wg_wat<-read_xlsx(here("./Data/FINAL/WG_Water_Data.xlsx"), sheet = "Data")
d_wg_sed<-read_xlsx(here("./Data/FINAL/WG_Sediment_Data.xlsx"), sheet = "Data")
...
# standardize PFAS names, take out `:`
names(d_jba_wat)<-gsub(":", "", names(d_jba_wat), fixed = TRUE)
names(d_jba_sed)<-gsub(":", "", names(d_jba_sed), fixed = TRUE)
names(d_jba_biota)<-gsub(":", "", names(d_jba_biota), fixed = TRUE)
names(d_wg_wat)<-gsub(":", "", names(d_wg_wat), fixed = TRUE)
...
# the unique PFAS in each compartment and each system 
...
pfas_wg_biota <- d_wg_biota %>%
  dplyr::select(c(PFBA : `11clPF3OUdS2`)) %>% names()
...
# all(wgPFAS[order(wgPFAS)] == jbaPFAS[order(jbaPFAS)]) # sanity check all JBA and WG PFAS are the same
```

```{r data organize last touches}
# filter out the original data sets
# and turn date format as needed
#  WG
d_wg_wat<-d_wg_wat %>% 
  dplyr::select(SampleID, SampleDate, Precipitation, Location, all_of(wgPFAS))
d_wg_sed<-d_wg_sed %>% 
  dplyr::select(SampleID, SampleDate, Location, all_of(wgPFAS))
d_wg_biota<-d_wg_biota %>% 
  dplyr::select(SampleID, SampleDate, Species, Sex,
         Tissue, Weight, Length, all_of(wgPFAS))

# JBA
d_jba_wat<-d_jba_wat %>% 
  dplyr::select(SampleID, SampleDate, Precipitation, Location, all_of(jbaPFAS))
d_jba_sed<-d_jba_sed %>% 
  dplyr::select(SampleID, SampleDate, Location, all_of(jbaPFAS))
d_jba_biota<-d_jba_biota %>% 
  dplyr::select(SampleID, SampleDate, CommonName, Sex,
...
write.csv(wg_all,
          file = here("./Data/SERDP_report_support/exports/wg_compiled_jan2025.csv"),
          row.names = FALSE)

write.csv(jba_all,
          file = here("./Data/SERDP_report_support/exports/jba_compiled_jan2025.csv"),
          row.names = FALSE)
...
rbind(jba_all, wg_all) %>% 
  ...
  as.data.frame() %>% 
  write.csv( "./Data/SERDP_report_support/exports/Summary_nPFAS.csv", row.names = FALSE)
...
jba_all %>% 
  ...
  as.data.frame() %>% 
  write.csv( "./Data/SERDP_report_support/exports/Summary_n_byPFAS_JBA.csv", row.names = FALSE)
...
wg_all %>% 
  ...
  pivot_wider(names_from = Media, values_from = `n samples`) %>% 
  as.data.frame() %>% 
  write.csv( "./Data/SERDP_report_support/exports/Summary_n_byPFAS_WG.csv", row.names = FALSE)
...
# order for x axis: 
# Spill Ripp Down Exit

# mutate(name = fct_relevel(Spp_Loc, 
#             "spill", "ripp", "down", "exit"))
...
# WATER -----
d_jba_wat_noSum$Spp_Loc = factor(d_jba_wat_noSum$Spp_Loc, levels = c("spill", "ripp", "down", "exit"))
d_jba_sed_noSum$Spp_Loc = factor(d_jba_sed_noSum$Spp_Loc, levels = c("spill", "ripp", "down", "exit"))
...
sumJBA<-rbind(jba_all) %>% 
  filter(Media == "water") %>% 
  mutate(SampleID = factor(SampleID)) %>% 
  dplyr::group_by(Media,SampleID, SampleDate, Seasonality) %>% 
...
wgEquil<-ggplot(sumWG, aes(x=as.Date(SampleDate), y=SumPFAS,
    ...
    theme_bw()+
    ylim(0, 10000)+
    geom_hline(yintercept = 1000, color = "grey", linewidth = 0.2, linetype = "dashed")+
...
jba_baf<-d_jba_baf %>% 
  ...
  theme(legend.direction = "vertical", 
        ...
        legend.background = element_blank(),
        axis.text.x = element_text(angle = 45, hjust = 1, vjust =1))
ggsave(jba_baf,filename = "./Figures/SERDP/JBA_BAF_mean_season.png",
...
wg_baf<-d_wg_baf %>% 
  ...
  theme(legend.direction = "vertical", 
        ...
        legend.background = element_blank(),
        axis.text.x = element_text(angle = 45, hjust = 1, vjust =1))
ggsave(wg_baf,filename = "./Figures/SERDP/WG_BAF_mean_season.png",
       width = 13, height = 6.5, units = "in")
